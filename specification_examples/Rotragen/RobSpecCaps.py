...
if __name__ == '__main__':
    ...
    targetpos = np.array([0.50,0.00,0.30])
    obstablepos = np.array([0.5, 0.00,0.40]) 
    ...
    joinstpos0 = np.array([-0.0] +2*[0.0] + [-1.0] + [1.00] + 2*[0.0])
    targetpos = np.array([0.50,0.00,0.30])
    obstablepos = np.array([0.5, 0.00,0.50]) 
    obstaclerad = np.array([0.20])
    global_params =np.hstack((joinstpos0, targetpos, obstablepos, obstaclerad))
    opti.set_value(optibuilder.global_params_in, global_params)
    opti.set_initial(optibuilder.x_vars, res.value(optibuilder.x_vars))
    opti.set_initial(optibuilder.u_vars, res.value(optibuilder.u_vars))
    res = opti.solve()
