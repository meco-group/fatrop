...
FatropAlg::FatropAlg(
    ...
    const shared_ptr<FatropAlg> &resto_alg)
...
{
    return optimize(mu0_);
}
fatrop_int FatropAlg::optimize(double mu0)
{
    ...
    for (fatrop_int i = 0; i < maxiter; i++)
    {
        ...
        if (ls == 0)
        {
            ...
            if (ret != 0)
            {
                printer_->level(1) << "error: fatrop failed to find a solution" << endl;
                return 1;
            }
            else
            {
...
fatrop_int FatropAlg::solve_resto_alg(double mu)
{
    ...
    resto_alg_->cv_orig_tol = 0.1*fatropdata_->constr_viol_sum_curr();
    ...
    resto_alg_->fatropnlp_->set_rho(100.*std::max(std::abs(fatropdata_->obj_curr), 1.0));
    int ret = resto_alg_->optimize(fatropdata_->constr_viol_max_curr());
