...
void Journaller::print_iterations(bool no_header)
{
    if (printer_->print_level() < 1)
        return;
    if (!no_header && print_count == 0)
    {
        printer_->printf(" it   obj            cv        du        lg(mu) reg  alpha_du  alpha_pr  ls\n");
    }
    for (std::vector<double>::size_type i = print_count; i < iterationdata.size(); i++)
    {
        ...
        if (iterationdata_i.reg == 0.0)
        {
            printer_->printf("%3d %c %.7e  %.2e  %.2e  %4.1f   -.-  %.2e  %.2e  %d%c \n", iterationdata_i.iter, iterationdata_i.resto?'R':' ' , iterationdata_i.objective, iterationdata_i.constraint_violation, iterationdata_i.du_inf, log10(iterationdata_i.mu), iterationdata_i.alpha_du, iterationdata_i.alpha_pr, abs(iterationdata_i.ls), iterationdata_i.type);
        }
        else
        {
            printer_->printf("%3d %c %.7e  %.2e  %.2e  %4.1f  %4.1f  %.2e  %.2e  %d%c \n", iterationdata_i.iter, iterationdata_i.resto?'R':' ', iterationdata_i.objective, iterationdata_i.constraint_violation, iterationdata_i.du_inf, log10(iterationdata_i.mu), log10(iterationdata_i.reg), iterationdata_i.alpha_du, iterationdata_i.alpha_pr, abs(iterationdata_i.ls), iterationdata_i.type);
        }
    }
