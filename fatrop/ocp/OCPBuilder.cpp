...
int OCPSolutionSampler_old::Size()
{
    return K_ * eval_->Size();
}
int OCPSolutionSampler_old::n_rows()
{
    return eval_->n_rows();
}
int OCPSolutionSampler_old::n_cols()
{
    return eval_->n_cols();
}
int OCPSolutionSampler_old::K()
{
    return K_;
}
...
vector<double> OCPSolutionSampler_old::Sample()
{
...
shared_ptr<FatropApplication> OCPBuilder::Build()
{
    ...
    ocptemplatebasic = make_shared<BasicOCP>(nu, nx, ngI, ng, ngF, ng_ineqI, ng_ineq, ng_ineqF, no_stage_params, no_global_params, K,
                                               BAbtf,
                                               bkf,
                                               RSQrqtIf,
                                               rqIf,
    ...
    {
        sampler_map[sampler_name] = make_shared<OCPSolutionSampler_old>(GetSamplerCustom(sampler_name));
    }
    for (auto var_name : json_spec["states_offset"].as_object().keys())
    {
        sampler_map[string("state_") + var_name] = make_shared<OCPSolutionSampler_old>(GetSamplerState(var_name));
    }
    for (auto var_name : json_spec["controls_offset"].as_object().keys())
    {
        sampler_map[string("control_") + var_name] = make_shared<OCPSolutionSampler_old>(GetSamplerControl(var_name));
    }
...
OCPSolutionSampler_old OCPBuilder::GetSamplerState(const string &variable_name)
{
    assert(solver_built);
    vector<int> in;
    vector<int> out;
