...
for step in range(N_steps):
    ddq_left, mechanism_left, links_left = walker2d_dynamics.ode_left_contact(q, qd, tau, 2*step*0.25)
    ddq_right, mechanism_right, links_right = walker2d_dynamics.ode_right_contact(q, qd, tau, (2*step+1)*0.25)
    ...
    stage_left.at_tf().subject_to(foot_right.left.origin() == [(2*step+1)*0.25, 0.0])
    stage_left.at_tf().subject_to(cs.sin(foot_right.left.theta) == 0.) # theta = 0. + k * pi
    if step != N_steps - 1:
        foot_left = links_right[-1]
        stage_right.at_tf().subject_to(foot_left.left.origin() == [(2*step+2)*0.25, 0.0])
