...
namespace fatrop{
    namespace spectrop
    {
            FatropStageEval FatropStageEval::create(const Stage &stage, const cs::Dict& opts)
            {
                ...
                ret.L = CasadiFEWrap(cs::Function("L", stage_syms, {sq.L}, optss), expand, jit, jit_options_);
                auto rq_sym = cs::MX();
                auto RSQ_sym = cs::MX::hessian(sq.L, ux, rq_sym);
                ret.rq = CasadiFEWrap(cs::Function("rq", stage_syms, {cs::MX::densify(rq_sym)}, optss), expand, jit, jit_options_);
                // if (sq.nxp1 > 0)
                {
                ...
                // if(sq.ng_eq > 0)
                {
                    ...
                    ret.Ggt_equality = CasadiFEWrap(cs::Function("Ggt_equality", stage_syms, {cs::MX::densify(Ggt_equality)}, optss), expand, jit, jit_options_);
                    ret.g_equality = CasadiFEWrap(cs::Function("g_equality", stage_syms, {cs::MX::densify(g_eq)}, optss), expand, jit, jit_options_);
                    auto rq_eq = cs::MX();
                ...
                // if(sq.ng_ineq>0)
                {
                    ...
                    ret.Ggt_inequality = CasadiFEWrap(cs::Function("Ggt_inequality", stage_syms, {cs::MX::densify(Ggt_inequality)}, optss), expand, jit, jit_options_);
                    ret.g_inequality = CasadiFEWrap(cs::Function("g_inequality", stage_syms, {cs::MX::densify(g_ineq)}, optss), expand, jit, jit_options_);
                    auto rq_ineq = cs::MX();
