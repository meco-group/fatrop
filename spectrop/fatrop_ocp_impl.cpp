...
namespace fatrop{
    namespace spectrop
    {
            FatropStageEval FatropStageEval::create(const Stage &stage, const cs::Dict& opts)
            {
                auto optss = opts;
                bool expand =cs::get_from_dict(optss, "post_expand", true);
                bool jit =cs::get_from_dict(optss, "jit", true);
                // optss.at("jit") = false;
                ...
                ret.L = CasadiFEWrap(cs::Function("L", stage_syms, {sq.L}, optss), expand, jit);
                auto rq_sym = cs::MX();
                auto RSQ_sym = cs::MX::hessian(sq.L, ux, rq_sym);
                ret.rq = CasadiFEWrap(cs::Function("rq", stage_syms, {cs::MX::densify(rq_sym)}, optss), expand, jit);
                // if (sq.nxp1 > 0)
                {
                ...
                // if(sq.ng_eq > 0)
                {
                    ...
                    ret.Ggt_equality = CasadiFEWrap(cs::Function("Ggt_equality", stage_syms, {cs::MX::densify(Ggt_equality)}, optss), expand, jit);
                    ret.g_equality = CasadiFEWrap(cs::Function("g_equality", stage_syms, {cs::MX::densify(g_eq)}, optss), expand, jit);
                    auto rq_eq = cs::MX();
                ...
                // if(sq.ng_ineq>0)
                {
                    ...
                    ret.Ggt_inequality = CasadiFEWrap(cs::Function("Ggt_inequality", stage_syms, {cs::MX::densify(Ggt_inequality)}, optss), expand, jit);
                    ret.g_inequality = CasadiFEWrap(cs::Function("g_inequality", stage_syms, {cs::MX::densify(g_ineq)}, optss), expand, jit);
                    auto rq_ineq = cs::MX();
                    auto RSQ_ineq = cs::MX::hessian(cs::MX::dot(g_ineq, lam_g_inequality), ux, rq_ineq);
                    rq_sym += rq_ineq;
                    RSQ_sym += RSQ_ineq;
                }
                ret.RSQrqt = CasadiFEWrap(cs::Function("RSQrqt", {sq.u, sq.x, lam_dynamics, lam_g_equality, lam_g_inequality, sq.p_stage, sq.p_global}, {cs::MX::densify(cs::MX::horzcat({RSQ_sym, rq_sym}).T())}, optss), expand, jit);
