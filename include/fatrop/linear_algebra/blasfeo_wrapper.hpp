...
namespace fatrop
{
    static inline void blasfeo_daxpby_debug(int m, double alpha, blasfeo_dvec *x, int xi, double beta, blasfeo_dvec *y, int yi, blasfeo_dvec *z, int zi)
    {
        fatrop_dbg_assert(m > 0 && "Vector size must be positive");
        fatrop_dbg_assert(xi >= 0 && yi >= 0 && zi >= 0 && "Vector indices must be non-negative");
        fatrop_dbg_assert(xi + m <= x->m && yi + m <= y->m && zi + m <= z->m && "Vector indices plus size must not exceed vector dimensions");
        blasfeo_daxpby(m, alpha, x, xi, beta, y, yi, z, zi);
    }

    static inline void blasfeo_daxpy_debug(int m, double alpha, blasfeo_dvec *x, int xi, blasfeo_dvec *y, int yi, blasfeo_dvec *z, int zi)
    {
        fatrop_dbg_assert(m > 0 && "Vector size must be positive");
        fatrop_dbg_assert(xi >= 0 && yi >= 0 && zi >= 0 && "Vector indices must be non-negative");
        fatrop_dbg_assert(xi + m <= x->m && yi + m <= y->m && zi + m <= z->m && "Vector indices plus size must not exceed vector dimensions");
    ...
    static inline void blasfeo_dvecse_debug(int m, double alpha, blasfeo_dvec *x, int xi)
    {
        ...
        fatrop_dbg_assert(xi + m <= x->m && "Vector index plus size must not exceed vector dimension");
    ...
    static inline void blasfeo_dveccpsc_debug(int m, double alpha, blasfeo_dvec *x, int xi, blasfeo_dvec *y, int yi)
    {
        fatrop_dbg_assert(m > 0 && "Vector size must be positive");
        fatrop_dbg_assert(xi >= 0 && yi >= 0 && "Vector indices must be non-negative");
        fatrop_dbg_assert(xi + m <= x->m && yi + m <= y->m && "Vector indices plus size must not exceed vector dimensions");
    ...
    static inline void blasfeo_dvecmul_debug(int m, blasfeo_dvec *x, int xi, blasfeo_dvec *y, int yi, blasfeo_dvec *z, int zi)
    {
        fatrop_dbg_assert(m > 0 && "Vector size must be positive");
        fatrop_dbg_assert(xi >= 0 && yi >= 0 && zi >= 0 && "Vector indices must be non-negative");
        fatrop_dbg_assert(xi + m <= x->m && yi + m <= y->m && zi + m <= z->m && "Vector indices plus size must not exceed vector dimensions");
