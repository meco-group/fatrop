...
namespace fatrop
{
    ...
    template <typename ProblemType> void IpInitializer<ProblemType>::initialize_slacks()
    {
        ...
        auto res_single_lower_bounded = max(
            res_unprojected, lower_bounds + bound_push * max(VecRealScalar(number_of_slacks, 1.0),
                                                             abs(lower_bounds)));
        auto res_single_upper_bounded = min(
            res_unprojected, upper_bounds - bound_push * max(VecRealScalar(number_of_slacks, 1.0),
                                                             abs(upper_bounds)));
        auto res =
