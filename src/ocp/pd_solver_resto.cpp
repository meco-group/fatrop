...
void PdSolverResto<OcpType>::reduce(LinearSystem<PdSystemResto<OcpType>> &ls)
{
    ...
    VecRealView rhs_cn = ls.rhs_cu_.block(info.number_of_eq_constraints, info.offset_n);
    VecRealView P = ls.Sl_i_.block(info.number_of_eq_constraints, info.offset_p);
    VecRealView N = ls.Su_i_.block(info.number_of_eq_constraints, info.offset_n);
    VecRealView Zp = ls.Zl_i_.block(info.number_of_eq_constraints, info.offset_p);
    VecRealView Zn = ls.Zu_i_.block(info.number_of_eq_constraints, info.offset_n);

    f_pp_ = f_p - 1.0 / P * rhs_cp;
    f_nn_ = f_n - 1.0 / N * rhs_cn;
    Xpm1_ = P / (P * ls.D_x_ + Zp);
    Xnm1_ = N / (N * ls.D_x_ + Zn);
...
void PdSolverResto<OcpType>::dereduce(LinearSystem<PdSystemResto<OcpType>> &ls, VecRealView &x)
{
    ...
    VecRealView orig_primal_x =
        x.block(info.number_of_primal_variables, info.pd_orig_offset_primal);
    VecRealView orig_primal_s = x.block(info.number_of_slack_variables, info.pd_orig_offset_slack);
    VecRealView orig_mult = x.block(info.number_of_eq_constraints, info.pd_orig_offset_mult);
    VecRealView orig_zl = x.block(info.number_of_slack_variables, info.pd_orig_offset_zl);
    VecRealView orig_zu = x.block(info.number_of_slack_variables, info.pd_orig_offset_zu);
    // set x
    ...
    VecRealView x_s =
        orig_primal_s.block(info.number_of_slack_variables_resto, info.pd_resto_offset_slack);
    ...
    VecRealView x_zl = orig_zl.block(info.number_of_slack_variables_resto, info.pd_resto_offset_zl);
