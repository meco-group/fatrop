...
LinearSystem<PdSystemResto<DenseType>>::LinearSystem(
    const ProblemInfo<DenseType> &info, Jacobian<DenseType> &jac, Hessian<DenseType> &hess,
    ...
    VecRealView &rhs_f_x, VecRealView &rhs_f_s, VecRealView &rhs_g, VecRealView &rhs_cl,
    VecRealView &rhs_cu)
    : info_(info), m_(LinearSystem<PdSystemResto<DenseType>>::m(info)), jac_(jac), hess_(hess),
...
Index LinearSystem<PdSystemResto<DenseType>>::m(const ProblemInfo<DenseType> &info)
{
    return info.number_of_tangent_variables + info.number_of_eq_constraints +
           3 * info.number_of_slack_variables_resto;
}

void LinearSystem<PdSystemResto<DenseType>>::get_rhs(VecRealView &out)
{
...
void LinearSystem<PdSystemResto<DenseType>>::set_rhs(const VecRealView &in)
{
...
void LinearSystem<PdSystemResto<DenseType>>::apply_on_right(const VecRealView &x, Scalar alpha,
                                                            const VecRealView &y,
                                                            VecRealView &out)
{
    ...
    VecRealView mult_eq = mult.block(info_.dims.ng, 0);
    ...
    VecRealView out_mult_eq = out_mult.block(info_.dims.ng, 0);
    ...
    out_s_p.block(info_.dims.ng, 0) =
        out_s_p.block(info_.dims.ng, 0) +
        mult.block(info_.dims.ng, 0);
    out_s_n.block(info_.dims.ng, 0) =
        out_s_n.block(info_.dims.ng, 0) -
        mult.block(info_.dims.ng, 0);
    ...
    if (!De_is_zero_)
        out_mult_eq =
            out_mult_eq - D_e_.block(info_.dims.ng, 0) * mult_eq;
    out_mult_ineq =
