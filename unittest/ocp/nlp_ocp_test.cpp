...
{
    VecRealAllocated res(nlp_ocp.nlp_dims().number_of_eq_constraints + nlp_ocp.nlp_dims().number_of_ineq_constraints);
...
TEST_F(NlpOcpTest, TestObjectiveGradient)
{
    ...
    Scalar objective_scale = 1.0;
    EXPECT_NO_THROW(nlp_ocp.eval_objective_gradient(ocp_info, objective_scale, primal_x, grad_x, grad_s));
    // Add more specific checks here if needed
}
...
TEST_F(NlpOcpTest, TestObjective)
{
    ...
    Scalar objective_scale = 1.0;
    EXPECT_NO_THROW(nlp_ocp.eval_objective(ocp_info, objective_scale, primal_x, primal_s, objective_value));
    // Add more specific checks here if needed
}
